  ...
  if (millis() - sendDataPrevMilli > 2000){
      ...
      if (scale.is_ready()) {
          ...
          if(weight >= 0.100 && initialWeightValue == 0){
              ...
              weightValue = weight;
              Firebase.setFloat(firebaseData, path + "/scaleSensor", weightValue);
              Firebase.setInt(firebaseData, path + "/numberOfObjects", 1);
              contor++;
          }else if(weight >= 0.100 && (weight >= (contor*weightValue - weightValue/4)) && (weight <= (contor*weightValue + weightValue/4))){
              Firebase.setFloat(firebaseData, path + "/scaleSensor", weightValue);
              Firebase.setInt(firebaseData, path + "/numberOfObjects", contor);
              contor++;
          }else if((weight - (contor-1)*weightValue < -weightValue/4) && (weight + weightValue/4 >= (contor-1)*weight - weightValue) && contor > 2 && weight >= 0.100 ){
              Firebase.setFloat(firebaseData, path + "/scaleSensor", weightValue);
              contor--;
              Firebase.setInt(firebaseData, path + "/numberOfObjects", contor - 1);
